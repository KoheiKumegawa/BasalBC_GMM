...
#--------- analysis ---------#
bamList <- list.files("data/", pattern = "Hs578T") %>% grep("bam$", ., value = T)
names(bamList) <- c("Hs578T-siFOXD1-1-1","Hs578T-siFOXD1-1-2","Hs578T-siFOXD1-2-1","Hs578T-siFOXD1-2-2","Hs578T-siNC-1-1","Hs578T-siNC-1-2")
...
summitPeaks <- list.files("data/", pattern = c("Hs578T")) %>% grep("summits", ., value = T)
...
names(reproduciblePeaks) <- paste0("Hs578T_K27AC_", c(1:length(reproduciblePeaks)))
...
saveRDS(se, "rds/Hs578T_K27AC_se_v2.rds")
